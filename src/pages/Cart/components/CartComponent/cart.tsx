...
import { Box, Title, Text, ScrollView, Cover, Button, Touchable } from "../../../../components"
...
export function CartComponent({changeTab}: CartComponentProps) {
	return (
		<Box hasPadding mt="15px" height="auto">
			...
			<ListItems>
				{array?.map((item) => (
					<Item key={item.id}>
						...
						<Box style={{ marginLeft: 20, marginBottom: 20 }}>
							...
							<Box height="35px" width="100%" row>
								<Box>
									<Text color="dark">Size: <Text>{item.size}</Text></Text>
									<Text color="dark">Quantity: <Text>{item.quantity}</Text></Text>
								</Box>
								<Box align="flex-end" justify="flex-end">
									<Touchable justify="flex-end"><Text color="primary">remove</Text></Touchable>
								</Box>
							</Box>
						</Box>
					</Item>
			...
			<PaymentInfo width="100%">
				...
				<Button background="primary" width="100%" mt="40px" onPress={() => changeTab({ label: 'PAYMENT', value: 'payment' })}>
